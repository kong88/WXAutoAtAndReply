...
//                hasAction = false;
    ...
    private void fillAndSend(String string) {
        AccessibilityNodeInfo rootNode = getRootInActiveWindow();
        if (rootNode == null) {
            return;
        }
        Log.d("maptrix", " ===============String:" + string);
        boolean flag = findEditText(rootNode, string);
//        Log.d("maptrix", " ===============flag:" + flag);
//        if (flag) {
//            send();
//        }
    }
    ...
    private void findNickname(String clickId) {
        Log.i("maptrix", "================== text:  findNickname()");
        AccessibilityNodeInfo rootNode = getRootInActiveWindow();
        if (rootNode == null) {
            return;
        }
        ...
        for (AccessibilityNodeInfo nodeInfo : list) {
            if ("android.widget.TextView".equals(nodeInfo.getClassName())) {
                Log.i("maptrix", "================== text: " + nodeInfo.getText());

                    if (nodeInfo.getText() == null || TextUtils.isEmpty(nodeInfo.getText().toString())) {
                        continue;
                    }
                    String content = nodeInfo.getText().toString();
                    regexMatcher(content);
            }
        }

    }

    public static final String REGEX_1 = "^\"([^\"]+?)\"邀请\"([^\"]+?)\"";
    public static final String REGEX_2 = "^\"([^\"]+?)\"通过扫描";

    protected void regexMatcher(String string) {
        if (TextUtils.isEmpty(string)) {
            return;
        }
//        String str1 = "\"kong1\"邀请\"kong target\" 加入了群聊";
//        String str2 = "\"kong target\"通过扫描\"kong\"分享的二维码加入了群聊";
        ...
        while (matcher1.find()) {
            Log.e("maptrix", "find111111111111111111111111111111");
            if (matcher1.groupCount() == 2) {
                String atNickname = matcher1.group(2);
                Log.e("maptrix", "find atNickname:" + atNickname);
                fillAndSend(String.format("@%s, 我是自动回复", atNickname));
            }
        }
        while (matcher2.find()) {
            Log.e("maptrix", "find 2222222222222222222");
